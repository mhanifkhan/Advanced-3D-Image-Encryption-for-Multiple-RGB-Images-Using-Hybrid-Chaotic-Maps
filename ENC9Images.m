function res = ENC9Images(bigimg)
...
%%%%%%%%%%%%%%%% End of Keys   %%%%%%%%%%%%%%%%%%%%%%%
                ...
                nn(1) = n0;
                 ...
                 for i = 1 : m*n*p+500-1    % -1 is used because first time the value is already assigned to u, v, w, i.e. u(1), v(1), w(1) it will start form u(2)...
...
for i = 1:m*n*p
 ...
 confuse = uint8(bigimg);
 
 C1 = confuse(:,:,1);  % Separating First plan
 C2 = confuse(:,:,2);  % Separating First plan
 C3 = confuse(:,:,3);  % Separating First plan
 C4 = confuse(:,:,4);  % Separating First plan
 C5 = confuse(:,:,3);  % Separating First plan
 C6 = confuse(:,:,4);  % Separating First plan
 C7 = confuse(:,:,3);  % Separating First plan
 C8 = confuse(:,:,4);  % Separating First plan
 C9 = confuse(:,:,3);  % Separating First plan


%  bigconf_all = [C1,  C2; C3, C4]; % Merging all images to single big image

 bigconf_all = [C1,  C2, C3; C4, C5, C6; C7, C8, C9]; % Merging all images to single big image
 
 imwrite(bigconf_all, 'bigCONF_1024.tiff','tiff');
